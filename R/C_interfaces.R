...
#' 
#' Assume given two sorted one-dimensional arrays \code{a} and \code{b}. For each element \code{a[i]}, determine the maximum index \code{j} in \code{b} such that \code{b[j] <= a[i]}, (\code{NA} if not found).
#' 
...
#' @param eps	tolerance for numerical noise, relative to the largest absolute elements in \code{a} and \code{b}
#' 
#' @examples
...
sorted_array_position <- function(a, b, eps=1e-9)
{
  ...
  eps_absolute <- eps * max(a, b, 1, na.rm=TRUE)
